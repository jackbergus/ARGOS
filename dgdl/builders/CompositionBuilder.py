...
class CompositionBuilder(BaseDGDLBuilder):
    ...
    def enterStore(self, ctx):
        ...
        if owner.SHARED() is not None:
            owners = [x.playerID for x in self.composition["players"]]
        else:
            owners = [o.getText() for o in owner.playerID()]
        structure = ctx.storeStructure().getText()
        visibility = ctx.visibility()
        if visibility is not None:
            if visibility.PUBLIC() is not None:
                visibility = "public"
            elif visibility.PRIVATE() is not None:
                visibility = "private"
    ...
    def enterExtURImap(self, ctx):
        id = ctx.extURIID().getText()
        uri = ctx.extURI().STRINGLITERAL().getText()
