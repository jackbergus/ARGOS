...
def handle_move_effect(dialogue, effect, data=None):
    ...
    if effect.action == "add":
        if dialogue.content_source is not None:
            raise Exception("Unexpected call for soemthing not working now! Shame on you...")
    ...
    if storeID in dialogue.stores:
        ...
        if data is not None:
            if (isinstance(owner, list) and not data["speaker"] in owner):
                return
            if ((not isinstance(owner, list)) and (owner is not None) and (data["speaker"] != dialogue.stores[storeID].owner)):
                return
